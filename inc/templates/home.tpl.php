...
<?php foreach ($articlesList as $articleId => $articleObject) : ?>
    <article class="card">
        <div class="card-body">
            ...
            <p class="infos">
                Posté par <a href="#" class="card-link"><?= $dataAuthorsList[$articleObject->author_id]->firstname ?></a> le <time datetime="<?= $articleObject->date ?>"><?= $articleObject->getDateFr(); ?></time> dans <a href="#"
                class="card-link">#
                <?= $dataCategoriesList[$articleObject->category_id]->category ?></a>
            </p>
        </div>
    </article>
<?php endforeach; ?>
